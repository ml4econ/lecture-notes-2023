...
The following code chunk utilizes the `pivot_longer()` function from the `{tidyr}` package to transform the data from wide to long format. This step will be useful for plotting the data later on:
```{r}
fgl_long <- 
  fgl_wide %>% 
...
For example, our model correctly classified 54 observations as `WinF` out of all the predicted `WinF` samples, resulting in a precision rate of $54/80 = 67.5\%$.

Similarly, the model correctly predicted 54 `WinF` samples out of the actual number of `WinF` samples, which is 64, resulting in a recall rate of $54/70 = 77.1\%$.
...
It is noticeable that the model struggles to differentiate between `Veh` and the other classes, while its classification for `Tabl` is nearly perfect.
]
.pull-right[
